...
<div class="card-wrapper">
    <div class="card">
        ...
        <!-- Card body -->
        <div class="card-body">
            <form action="{{ route('problems.update', $current_product->id) }}" method="POST" class="needs-validation" novalidate>
                @method('put')
                @csrf
                ...
                <div class="form-row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input name="amount" class="form-control @error('amount') is-invalid @enderror" id="amount" placeholder="Jumlah" value="{{ old('amount', $current_product->amount) }}" min="1" type="number">
                        <input name="old_amount" class="form-control" id="amount" placeholder="Jumlah" value="{{ $current_product->amount }}" min="1" type="hidden" readonly>
                        @error('amount')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control" id="description" rows="3">{{ $current_product->description }}</textarea>
                  </div>
                <button onclick="return confirm('Stok saat ini akan dikurangi sejumlah barang yang bermasalah, anda yakin?')" class="btn btn-primary float-right" type="submit">Perbarui</button>
            </form>
